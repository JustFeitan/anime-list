...
type updatedUserFields = Pick<IUser, 'username' | 'profileCover' | 'userAvatar'>;

const EditProfile: FC<EditProfileProps> = ({user, onSubmitHandler, ...props}) => {


    const [sizeErrors, setSizeErrors] = useState<{ error: string }[]>([]);
    const [avatar, setAvatar] = useState<string | null | ArrayBuffer>(user.userAvatar);
    const [profileCover, setProfileCover] = useState<string | null | ArrayBuffer>(user.profileCover);
        ...
        reader.onload = () => {
            ...
            img.onload = () => {
                ...
                if (img.height < 200 || img.width < 200) {
                    setSizeErrors([...sizeErrors, {error: 'Image size have to be at least 200px wight and 200px height'}])
                    return;
                } else {
        ...
        reader.onload = () => {
            ...
            img.onload = () => {
                if (img.height < 200 || img.width < 900) {
                    console.log('work', img.width, img.height)
                    setSizeErrors([...sizeErrors, {error: 'Image size have to be at least 200px wight and 900px height'}]);
                    return;
                } else {
    ...
    const {register, formState: {errors}, handleSubmit} = useForm<updatedUserFields>({
        defaultValues: {username: user.username}
    });
    const [updateUser, {isLoading}] = authApi.useUpdateUserMutation();

    //Add compare of user and updated user
    const onProfileChangesSubmit: SubmitHandler<updatedUserFields> = async (updatedUserFiles) => {
    ...
    return (
            ...
            <Form className='profile-edit' onSubmit={handleSubmit(onProfileChangesSubmit)}>
                <Input
                    label='Cover'
                ...
                <Input
                    ...
                    {...getAvatarInputProps()}
                />
                <Avatar {...getAvatarRootProps()} size={120} avatarImage={avatar as string}/>
