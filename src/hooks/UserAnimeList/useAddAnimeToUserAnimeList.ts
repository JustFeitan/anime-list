...
export const useAddAnimeToUserAnimeList = () => {
    const [addAnimeToStatusList, {isSuccess}] = userAnimeApi.useAddAnimeToStatusListMutation();
    const user = useAuth();
    return async (newStatus: SingleValue<UserAnimeStatusOption>, anime: IAnime) => {
        ...
        const response = await addAnimeToStatusList(newUserAnimeListItem).unwrap();
        if (response) {
