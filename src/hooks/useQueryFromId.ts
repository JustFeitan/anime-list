...
export function useQueryFromId<T extends Array<O>, O extends Partial<Pick<UserAnimeListItem, 'userId' | 'animeId' | 'id'>>>
(arrayWithIds: T | undefined, idFieldName: keyof O, limit?: number ) {
    return useMemo(() => {
        ...
        if (arrayWithIds !== undefined && arrayWithIds.length) {
            const query = arrayWithIds.map((obj) => `id=${obj[idFieldName]}`).join('&');
            params = {
                query: query,
                limit: limit || 10
            }
